...
#include "/home/dexin/source_code/prj_ros/fast_lio_ws/src/FAST_LIO/src/tools/data_dump_cloud.h"
...
void ImuProcess::Process(const MeasureGroup &                          meas,
  ...
  if (imu_need_init_) {
    ...
    if (init_iter_num > MAX_INI_COUNT) {
      ...
      ROS_INFO(
          "IMU Initial Done: Gravity: %.4f %.4f %.4f %.4f; state.bias_a: %.4f %.4f %.4f; "
          "state.bias_g: %.4f %.4f %.4f; acc covarience: %.8f %.8f %.8f; gry covarience: %.8f %.8f "
          "%.8f",
          imu_state.grav[0],
          imu_state.grav[1],
          imu_state.grav[2],
